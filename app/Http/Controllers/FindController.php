    ...
    public function getFindParking(Request $request)
{
    ...
    $sessions = ParkingSession::where('card_id', $cardId)
                             ->orderBy('id', 'desc')
                             ->get();
    ...
    foreach ($sessions as $session) {
        ...
        if ($parkingSlot) {
            ...
            $parkingSlots[] = [
                'current_slot' => $currentSlot['slot_number'] . $currentSlot['slot_code'],
                'neighbor_slots' => array_map(function($slot) {
                    return $slot['slot_number'] . $slot['slot_code'];
                }, $neighborSlots),
            ];
